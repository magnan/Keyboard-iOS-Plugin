...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView {
    NSLog(@"scroll");
    if(lockScroll)  [self.webView.scrollView setContentOffset:origin animated:NO];
}
...
- (void) hideFormAccessoryBar:(CDVInvokedUrlCommand*)command
{
    ...
    if(![value boolValue])
    {
        /* UITextField *text = [[UITextField alloc] initWithFrame:CGRectMake(-100, -100, 50, 20)];
        [self.webView addSubview:text];
